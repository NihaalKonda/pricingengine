...
ticker = "AAPL" # specific asset to fetch data for
...
parameter_tuning_start = int(datetime(2000, 10, 19).replace(tzinfo=timezone.utc).timestamp())
parameter_tuning_end = int(datetime(2008, 12, 31).replace(tzinfo=timezone.utc).timestamp())

model_training_start = int(datetime(2009, 1,1).replace(tzinfo=timezone.utc).timestamp())
model_training_end = int(datetime(2016, 12, 31).replace(tzinfo=timezone.utc).timestamp())

backtest_start = int(datetime(2017, 1,1).replace(tzinfo=timezone.utc).timestamp())
